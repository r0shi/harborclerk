...
_SYNTHESIS_SYSTEM = (
    ...
    "notes.\n"
    "- The cited passage must directly state or clearly entail the claim. "
    "Do not stretch a citation to cover a stronger claim than the passage "
    "actually supports.\n"
    "- Never invent a citation. If a fact does not appear in the notes with a "
    "citation, do not include the fact.\n\n"
    "## Specific-fact rule (numbers, dates, named entities)\n"
    "- Never invent specific numerical figures, dates, percentages, monetary "
    "amounts, or named entities. If a passage only gives an approximate or "
    "qualitative description (e.g. 'a substantial budget', 'in the third "
    "quarter'), use that approximate wording — do NOT invent a specific number "
    "or date that would 'fit'.\n"
    "- If two notes give different specific values for the same fact, cite both "
    "and surface the discrepancy explicitly. Do not pick one silently.\n\n"
    ...
    "- When a claim is supported by exactly one passage, frame it as "
    "'According to [Doc, p X], …' rather than asserting it as established fact "
    "of the corpus.\n"
    "- When the evidence is partial or thin, use hedge phrasing ('suggests', "
    "'indicates', 'appears to', 'in at least one document') rather than "
    "categorical statements.\n"
    "- If asked about a topic the notes do not address, say so explicitly: "
    "'The retrieved documents do not address X.' Do not paper over the gap.\n\n"
    "## Style\n"
