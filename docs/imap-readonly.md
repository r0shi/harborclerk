...
2. **Subclass layer.** Harbor Clerk's `ReadOnlyIMAP4_SSL` overrides every mutating method on the underlying aioimaplib client (`select`, `store`, `copy`, `move`, `expunge`, `append`, `create`, `delete`, `rename`, `subscribe`, `unsubscribe`) to raise before any bytes leave the process. The polymorphic `uid()` entry point inspects its subcommand and refuses `STORE / COPY / MOVE / EXPUNGE`. A drift-guard test enumerates aioimaplib's public surface and fails CI if a new method is added that we haven't classified. `examine` is also overridden, but for bookkeeping rather than refusal: aioimaplib 2.0.1 does not enter the *selected* state on EXAMINE (only on `SELECT`, which this class blocks), so the override records the state transition the server has already made — and clears it again when the server refuses. Without it no post-selection command could run at all.
