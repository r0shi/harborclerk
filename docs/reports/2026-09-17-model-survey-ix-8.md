...
- **Run id:** `ix-8` · commit `v0.9.2-39-g79a5c73` · window: releases since 2026-07-22 (given with --since) · 27 vendors watched
...
chat template, about 15,500 likes and 8.2M GGUF downloads in six weeks. Its architecture is `qwen35`, which the pinned
...
`zai-org/GLM-5.3-Flash` (2,418 likes), whose architecture llama.cpp cannot load yet. That list also holds a few untagged research artefacts (a climate model, a question generator) that are examined because Mistral
...
`Edge0/Edge0-35B-A3B-preview` drew 3,288 likes in nine days from an unknown publisher. Read its card before adding
...
Screened only for reasons time can cure: no trusted Q4_K_M build yet, an architecture llama.cpp cannot load yet, or a template or context a publisher may fix. The next survey examines these again however old they are. A release drops off 120 days after its repo was created.
...
{
 "format": 1,
 "generated_at": "2026-09-17T20:21:15+00:00",
 "inputs": {
  "carried": {},
