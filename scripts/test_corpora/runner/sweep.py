...
def _settle_summaries(hc: HarborClerkClient, corpus_id: str, wait: bool, stall_seconds: float) -> None:
    ...
    but once a model is active they are generated by that model on its one slot, between the questions it is
    being measured on, and a model that runs later reads summaries an earlier one wrote. Every model measures
    the same corpus only when they are done (#717). ``wait=False`` proceeds and says so; metrics.csv then carries
    the backlog per unit."""
    ...
    if not hc.wait_for_summaries(max_stall_seconds=stall_seconds):
        raise RuntimeError(
    ...
    try:
        ...
        for corpus in ordered_corpora:
            ...
            for u in corpus_units:
                ...
                # --no-wait-for-summaries run says on every row what it ran beside.
                try:
                    summarize_backlog_at_start: int | str = hc.summarize_backlog()
