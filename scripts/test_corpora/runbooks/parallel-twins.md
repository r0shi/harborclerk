...
Pick this if you have two roughly comparable machines (both ≥ 32 GB unified memory, similar M-series generation) and want to roughly halve the wall-clock of Phases 4 and 5. Phase 0/1/6 stay on one designated **coordinator** machine; Phase 4 splits the the models between the two machines; Phase 5's top-2 parity step splits one model per machine. Both machines need enough memory headroom to run a ~35B-class model (Qwen 3.6 35B-A3B on the coordinator, Gemma 4 26B-A4B on the other) — if one machine can't, run the [single-machine topology](single.md) instead.
