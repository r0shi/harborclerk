...
export default function QueueToastPopup({ items, onDismiss }: QueueToastPopupProps) {
  ...
  return (
    ...
    >
      <div className="rounded-xl bg-(--bg-vibrancy) backdrop-blur-xl px-4 py-3 shadow-mac-lg ring-1 ring-(--color-border) cursor-pointer">
        {latest && (
          <>
            <div className="flex items-center space-x-2">
              <div className="h-2 w-2 animate-pulse rounded-full bg-amber-500" />
              <span className="text-sm font-medium text-(--color-text-primary) truncate max-w-[200px]">
                {latest.filename}
              </span>
              {items.length > 1 && (
                <span className="text-xs text-(--color-text-secondary)">+{items.length - 1} more</span>
              )}
            </div>
            <div className="text-xs text-(--color-text-secondary) mt-0.5 ml-4">
